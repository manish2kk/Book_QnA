#!/usr/bin/env python3
"""Interactive / one-shot completion with a pretrained word-level GPT (MTP speculative)."""
...
DEFAULT_CKPT_DIR = ROOT / "gpt_checkpoints" / "yoga_word_gpt"
...
def load_model(ckpt_dir: Path, device: torch.device) -> tuple[CharGPT, WordTokenizer]:
    ckpt_path = ckpt_dir / "best.pt"
    if not ckpt_path.exists():
        ckpt_path = ckpt_dir / "last.pt"
    if not ckpt_path.exists():
        raise FileNotFoundError(
            f"No checkpoint in {ckpt_dir}. Run: .venv/bin/python train_gpt_word.py"
        )
...
def main() -> int:
    ...
    print("WordGPT completion. Type a prompt and press Enter. Empty line / 'quit' to exit.")
    while True:
        try:
            prompt = input("\n>>> ")
