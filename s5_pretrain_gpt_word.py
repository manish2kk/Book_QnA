#!/usr/bin/env python3
"""Pretrain a word-level GPT on a book text file.

Builds vocabulary from all *.txt in --vocab-dir (default book1_txt).
Use --resume with the same --out-dir to continue on another book.
"""
...
DEFAULT_OUT = ROOT / "gpt_checkpoints" / "yoga_word_gpt"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Build word vocab from all *.txt in this folder (default: book1_txt).",
    )
    parser.add_argument(
        "--min-freq",
        type=int,
        default=2,
        help="Keep words/punct seen at least this often across vocab-dir (default 2).",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    # Word context is denser than chars → shorter default block.
    ...
    parser.add_argument("--n-mtp", type=int, default=3, help="Multi-token prediction heads (0 disables MTP / speculative drafts).",)
    parser.add_argument("--mtp-loss-weight", type=float, default=0.3, help="Weight for MTP auxiliary losses relative to next-token loss.",)
    parser.add_argument("--step-sleep", type=float, default=1.0, help="Seconds to pause after each training step (cools laptop; 0 = no pause).",)
    ...
    if args.resume:
        ...
        n_new = tokenizer.extend_with_files(vocab_files, min_freq=args.min_freq)
        ...
        tokenizer = WordTokenizer.from_files(vocab_files, min_freq=args.min_freq)
        config = GPTConfig(
        ...
        print(
            ...
            f"word vocab={tokenizer.vocab_size}, n_mtp={config.n_mtp}"
        )
    ...
    print(
        f"Corpus: {args.data.name} | tokens={n:,} | vocab={tokenizer.vocab_size} | "
